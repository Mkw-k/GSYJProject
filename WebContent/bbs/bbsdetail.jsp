...
<table class='table'>
	...
	<tr>
		<th>CONTENT</th><td colspan=2><img src="./upload/<%=bbs.getFilename() %>" style="width: 80%;">
			<br> 
			<%=bbs.getMycontent() %>
		</td>
	</tr>	
...
%>
	...
	<tr>
		...
		%>
		<td>	
		<input type='button' class='btn btn-secondary' data-toggle="modal" data-target="#myModal1" value='수정' onclick="commentSeq('<%=dto.getSeq()%>')">
		<input type='button' class='btn btn-secondary' data-toggle="modal" data-target="#myModal2" value='삭제' onclick="commentSeq('<%=dto.getSeq()%>')">
		</td>
	</tr>
	<%
		}
...
%>
	<input type="submit" class='btn btn-secondary'  value="수정" onclick="location.href='bbs?param=updateBbs&seq=<%=bbs.getSeq() %>'">
    <input type="button" class='btn btn-secondary'  value="삭제"  data-toggle="modal" data-target="#myModal3">
<%
}
%>
	<input type="button" class='btn btn-secondary'  value="목록" onclick="location.href='bbs?param=getPagingBbsList'">
</div>
<br>
</div>
</div>
  ...
  <div class="modal" id="myModal3">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <!-- Modal footer -->
        <div class="modal-footer">
          <button type="button" class="btn btn-danger" data-dismiss="modal" onclick="location.href='bbs?param=deleteBbs&seq=<%=bbs.getSeq() %>'">예</button>
          <button type="button" class="btn btn-danger" data-dismiss="modal">아니오</button>
        </div>
