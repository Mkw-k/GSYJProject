...
%>
	...
	<tr>
		<td><%=cDto.getId() %></td>
		<td>
		<input type='text' class='form-control' name='content' value='<%=cDto.getContent() %>'>
		</td>
		<!-- 로그인 아이디와 글쓴이가 같거나 매니저라면 수정/삭제 버튼 보이기 -->
		<%
		if(managerNum==1 || cDto.getId().equals(id)){
		%>
		<td>	
		<input type='button' class='btn btn-secondary' data-toggle="modal" data-target="#myModal1" value='수정' onclick="commentSeq('<%=cDto.getSeq()%>')">
		<input type='button' class='btn btn-secondary' data-toggle="modal" data-target="#myModal2" value='삭제' onclick="commentSeq('<%=cDto.getSeq()%>')">
		</td>
	</tr>
	<%
		}
...
if(dto.getMyid().equals(id) || managerNum == 1){
	%>	
	<button type="button" class='btn btn-secondary' onclick="deleteAf(<%=dto.getSeq() %>)">삭제하기 </button>
	&nbsp; | &nbsp;
	<button type="button" class='btn btn-secondary' onclick="updateAf(<%=dto.getSeq() %>)">수정하기  </button>
	&nbsp; | &nbsp;
	<%
	}
%>
	<button type="button" class='btn btn-secondary' onclick="goList(<%=dto.getSeq() %>)">목록으로 </button>
</div>
</div>
